...
#define BGM_INST(attn, env_on, env_step, env_speed, env_curve_id, pitch_curve_id, \
                 ...
                 sweep_step, sweep_speed, mode, macro_id) \
    { attn, env_on, env_step, env_speed, env_curve_id, pitch_curve_id, \
      vib_on, vib_depth, vib_speed, vib_delay, sweep_on, sweep_end, \
      sweep_step, sweep_speed, mode, macro_id }

/* Customize instrument presets here (used by BGM_OP_SET_INST).
 * Keep the list short to save ROM; ids are 0..count-1. */
static const BgmInstrument s_bgm_instruments[] = {
    /* 0: clean tone */
    BGM_INST(2, 0, 1, 1, 0, 0, 0, 0, 1, 0, 0, 1, 0, 1, 0, 0),
    /* 1: noise default */
    BGM_INST(4, 0, 1, 1, 0, 0, 0, 0, 1, 0, 0, 1, 0, 1, 1, 0),
    /* 2: bright lead (light env + vibrato) */
    BGM_INST(1, 1, 1, 2, 1, 0, 1, 2, 3, 2, 0, 1, 0, 1, 0, 0),
    /* 3: soft pad (slow env, mild vibrato) */
    BGM_INST(4, 1, 1, 4, 1, 0, 1, 1, 5, 4, 0, 1, 0, 1, 0, 0),
    /* 4: pluck (macro + env curve) */
    BGM_INST(2, 1, 1, 2, 1, 0, 0, 0, 1, 0, 0, 1, 0, 1, 0, 1),
    /* 5: bass (slow sweep down) */
    BGM_INST(3, 1, 1, 3, 0, 1, 0, 0, 1, 0, 1, 180, -2, 3, 0, 0),
    /* 6: bell (soft wobble) */
    BGM_INST(2, 1, 1, 2, 1, 3, 1, 1, 4, 2, 0, 1, 0, 1, 0, 0),
    /* 7: zap (fast fall) */
    BGM_INST(2, 1, 1, 1, 0, 4, 0, 0, 1, 0, 1, 220, -6, 2, 0, 1),
};
...
static int BufferPushIfChanged(u8 ch, const PsgCmd *cmd)
{
    ...
    if (s_psg_shadow[ch].valid &&
        s_psg_shadow[ch].b1 == cmd->b1 &&
        s_psg_shadow[ch].b2 == cmd->b2 &&
        s_psg_shadow[ch].b3 == cmd->b3) {
        return 0;
    }
...
static void BgmVoice_CommandFromState(BgmVoice *v, PsgCmd *cmd)
{
    ...
    if (v->mode == 1) {
        u8 val = (u8)((v->note_idx - 1) & 0x07);
        v->shadow_b1 = (u8)(v->freq_base | val);
        v->shadow_b2 = (u8)(v->freq_base | val);
        v->shadow_b3 = (u8)(v->attn_base | (v->attn_cur & 0x0F));
    } else {
        s16 vib_delta = 0;
        u16 div = v->tone_div;
        {
...
#endif
    if (!v->env_on && !(v->mode == 0 && (v->sweep_on || v->vib_on)) && !dirty) {
        return 0;
    }
    if (v->env_on) {
        if (v->env_counter == 0) {
...
void Bgm_Update(void)
{
    ...
    elapsed = (u8)(VBCounter - s_bgm_last_vbl);
    if (elapsed == 0) {
        return;
    }
